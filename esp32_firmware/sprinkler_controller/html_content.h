...
.nav-btn { flex: 1; padding: 10px 2px 14px; text-align: center; cursor: pointer; border: none; background: none; color: #607080; transition: color 0.2s; font-family: inherit; }
.nav-btn.active { color: #4fc3f7; }
.nav-btn:active { opacity: 0.7; }
.nav-icon { font-size: 1.4rem; display: block; margin-bottom: 2px; }
.nav-label { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; }

/* ── Weather page ─────────────────────────────────────── */
.wx-hero { background: #131f2e; border: 2px solid #1a3a5c; border-radius: 16px; padding: 24px 20px; text-align: center; margin-bottom: 14px; }
.wx-hero-icon { font-size: 4rem; margin-bottom: 8px; }
.wx-hero-temp { font-size: 3rem; font-weight: 700; color: #e0e0e0; margin-bottom: 4px; }
.wx-hero-desc { font-size: 1.1rem; color: #90caf9; }
.wx-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 14px; }
.wx-card { background: #131f2e; border: 2px solid #1a3a5c; border-radius: 14px; padding: 16px; }
.wx-card-label { font-size: 0.8rem; color: #607080; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; margin-bottom: 6px; }
.wx-card-val { font-size: 1.5rem; font-weight: 700; color: #4fc3f7; }
.wx-card-sub { font-size: 0.85rem; color: #607080; margin-top: 4px; }
.wx-card.warn { border-color: #e65100; }
.wx-card.warn .wx-card-val { color: #ffb74d; }
.wx-card.freeze { border-color: #1565c0; }
.wx-card.freeze .wx-card-val { color: #81d4fa; }
.wx-skip-box { border-radius: 12px; padding: 16px; margin-bottom: 14px; border: 2px solid #1a3a5c; background: #131f2e; }
.wx-skip-box.skipping { border-color: #e65100; background: #1a0e00; }
.wx-skip-box.active { border-color: #2e7d32; background: #0d1f0f; }
.wx-skip-label { font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; margin-bottom: 4px; }
.wx-skip-reason { font-size: 1rem; color: #e0e0e0; }
.wx-footer { text-align: center; font-size: 0.85rem; color: #546e7a; margin-bottom: 10px; }
.wx-no-data { text-align: center; padding: 48px 20px; color: #607080; }
...
<div class="page" id="page-weather">
  <div id="wx-page-nodata" class="wx-no-data" style="display:none">
    <div class="wx-no-data-icon">🌐</div>
    <p>No weather data yet.<br>Set your location in Settings<br>and wait a few seconds.</p>
  </div>
  <div id="wx-page-content" style="display:none">
    ...
    <div class="wx-grid">
      <div class="wx-card" id="wxp-rain-today-card">
        <div class="wx-card-label">Rain Today</div>
        <div class="wx-card-val" id="wxp-rain-today">--%</div>
        <div class="wx-card-sub">probability</div>
      </div>
      <div class="wx-card" id="wxp-rain-tomorrow-card">
        <div class="wx-card-label">Rain Tomorrow</div>
        <div class="wx-card-val" id="wxp-rain-tomorrow">--%</div>
        <div class="wx-card-sub">probability</div>
      </div>
      <div class="wx-card" id="wxp-min-card">
        <div class="wx-card-label">Min Temp Today</div>
        <div class="wx-card-val" id="wxp-min-temp">--°C</div>
        <div class="wx-card-sub" id="wxp-freeze-sub">overnight low</div>
      </div>
      <div class="wx-card">
        <div class="wx-card-label">Updated</div>
        <div class="wx-card-val" style="font-size:1.1rem" id="wxp-age">--</div>
        <div class="wx-card-sub"><button onclick="refreshWeatherPage()" style="background:none;border:none;color:#4fc3f7;cursor:pointer;font-size:0.85rem;padding:0;font-family:inherit">↻ Refresh</button></div>
      </div>
    </div>
...
async function updateWeatherPage() {
  try {
    const w = await GET('/api/weather');
    const nodata  = document.getElementById('wx-page-nodata');
    const content = document.getElementById('wx-page-content');
    ...
    document.getElementById('wxp-rain-today').textContent    = w.rain_prob_today + '%';
    document.getElementById('wxp-rain-tomorrow').textContent = w.rain_prob_tomorrow + '%';
    document.getElementById('wxp-rain-today-card').className    = 'wx-card' + (w.weather_enabled && w.rain_prob_today    >= w.rain_threshold ? ' warn' : '');
    document.getElementById('wxp-rain-tomorrow-card').className = 'wx-card' + (w.weather_enabled && w.rain_prob_tomorrow >= w.rain_threshold ? ' warn' : '');
    ...
    const lbl    = document.getElementById('wxp-skip-label');
    const reason = document.getElementById('wxp-skip-reason');
    if (!w.weather_enabled) {
      box.className = 'wx-skip-box';
      lbl.style.color = '#607080';
      lbl.textContent = 'Weather Skip — Disabled';
      reason.textContent = 'Enable in Settings to skip watering on rain or freeze days.';
    } else if (w.rain_prob_today >= w.rain_threshold || w.rain_prob_tomorrow >= w.rain_threshold) {
      box.className = 'wx-skip-box skipping';
      lbl.style.color = '#ffb74d';
      lbl.textContent = '⚠ Watering Will Be Skipped';
      reason.textContent = `Rain forecast: ${Math.max(w.rain_prob_today, w.rain_prob_tomorrow)}% ≥ ${w.rain_threshold}% threshold`;
    } else if (freezeRisk) {
      box.className = 'wx-skip-box skipping';
      ...
      lbl.textContent = '✓ Watering Will Run';
